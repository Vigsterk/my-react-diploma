...
`POST /cart/${cartId}` — добавить новый товар в коризну. Тут `${cartId}` — идентификатор корзины, может быть пустым.
...
`POST /cart/${cartId}` — удалить товар из коризны. Тут `${cartId}` — идентификатор корзины.
...
- `id` — идентификатор товара на сервере, _число_;
- `size` — размер товара, _строка_;
- `amount` — количество единиц товара, для удаления должно быть нулевым, _число_.
